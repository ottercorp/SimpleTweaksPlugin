...
public unsafe class ImprovedWorldVisit : UiAdjustments.SubTweak {
    ...
    private void SetupWorldTravelSelect(AtkUnitBase* unitBase) {
        ...
        var orderedWorlds = Service.Data.GetExcelSheet<World>()!.Where(w => w.DataCenter.RowId == currentDc && w.RowId is > 1000 and not 1200).OrderBy(w => w.Name.ExtractText()).Select(w => w.Name.ExtractText()).ToList();
